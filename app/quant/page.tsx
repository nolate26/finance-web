...
function fmtGR(v: number | null): React.ReactNode {
  if (v === null) return <span style={{ color: "#CBD5E1", fontSize: 10, fontStyle: "italic" }}>NR</span>;
  const pct = v * 100;
  const color = pct > 0 ? GREEN : pct < 0 ? RED : TEXT2;
  return <span style={{ color, fontWeight: 700 }}>{pct > 0 ? "+" : ""}{pct.toFixed(1)}%</span>;
}

function EarningsSurprisesTab() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch("/api/quant/earnings-surprises")
      .then((r) => r.json())
      .then((d: { data?: EarningsRow[]; dates?: string[] }) => {
        const rows = d.data ?? [];
        const dts  = d.dates ?? [];
        setAllData(rows);
        setDates(dts);
        if (dts.length > 0) setSelDate(dts[0]);
      })
      .finally(() => setLoading(false));
  }, []);
  ...
  const displayed = allData.filter((r) => {
    const matchDate   = selDate ? r.reportDate === selDate : true;
    const matchSearch = q ? r.tickerBloomberg.toLowerCase().includes(q) || r.quarter.toLowerCase().includes(q) : true;
    return matchDate && matchSearch;
  });

  const thStyle: React.CSSProperties = {
    padding: "5px 10px", textAlign: "center", fontSize: 9, fontWeight: 600,
    letterSpacing: "0.07em", textTransform: "uppercase", color: "#94A3B8",
    background: "#F8FAFC", borderBottom: "1px solid rgba(15,23,42,0.08)", whiteSpace: "nowrap",
  };
  const sepTh: React.CSSProperties = { ...thStyle, borderRight: "1px solid rgba(15,23,42,0.08)" };

  return (
    <div style={{ ...cardStyle }}>
      {/* Top bar */}
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14, flexWrap: "wrap" }}>
        <SLabel>Earnings Surprises — All Companies</SLabel>
        <div style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <input
            ...
            style={{
              ...
              color: "#334155", outline: "none", width: 140,
              fontFamily: "JetBrains Mono, monospace",
            }}
          />
          {/* Date picker */}
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ fontSize: 11, color: TEXT2, fontFamily: "JetBrains Mono, monospace" }}>Report date</span>
            <select
              value={selDate}
              onChange={(e) => setSelDate(e.target.value)}
              style={{
                background: CARD, border: `1px solid ${BORDER}`, borderRadius: 8,
                padding: "5px 10px", color: TEXT1, fontSize: 12,
          ...
          <span style={{ fontSize: 11, color: TEXT2, fontFamily: "JetBrains Mono, monospace" }}>
      ...
      {loading ? (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "64px 0", gap: 12 }}>
          <div style={{ width: 24, height: 24, borderRadius: "50%", border: `2px solid rgba(59,130,246,0.2)`, borderTopColor: BLUE, animation: "spin 0.8s linear infinite" }} />
          <span style={{ fontSize: 12, color: TEXT2, fontFamily: "JetBrains Mono, monospace" }}>Loading earnings data…</span>
        </div>
      ) : displayed.length === 0 ? (
        <div style={{ textAlign: "center", padding: "48px 0", color: TEXT2, fontSize: 13 }}>
          No earnings data{selDate ? ` for ${selDate}` : ""}{q ? ` matching "${query}"` : ""}.
        </div>
      ) : (
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              {/* Group row */}
              <tr>
                <th colSpan={3} style={{ padding: "6px 14px", background: "#F0F4FA", borderBottom: "1px solid rgba(15,23,42,0.08)", borderRight: "1px solid rgba(15,23,42,0.08)" }} />
                <th colSpan={3} style={{ padding: "6px 10px", textAlign: "center", fontSize: 9, fontWeight: 800, letterSpacing: "0.10em", textTransform: "uppercase", color: "#2B5CE0", background: "rgba(43,92,224,0.06)", borderBottom: "2px solid rgba(43,92,224,0.20)", borderRight: "1px solid rgba(15,23,42,0.08)" }}>Revenue</th>
                <th colSpan={3} style={{ padding: "6px 10px", textAlign: "center", fontSize: 9, fontWeight: 800, letterSpacing: "0.10em", textTransform: "uppercase", color: "#7C3AED", background: "rgba(124,58,237,0.06)", borderBottom: "2px solid rgba(124,58,237,0.20)", borderRight: "1px solid rgba(15,23,42,0.08)" }}>EBITDA</th>
                <th colSpan={3} style={{ padding: "6px 10px", textAlign: "center", fontSize: 9, fontWeight: 800, letterSpacing: "0.10em", textTransform: "uppercase", color: AMBER, background: "rgba(217,119,6,0.06)", borderBottom: "2px solid rgba(217,119,6,0.20)" }}>Net Income</th>
              </tr>
              {/* Sub-header row */}
              <tr>
                <th style={{ ...thStyle, textAlign: "left", padding: "5px 14px" }}>Ticker</th>
                <th style={{ ...thStyle, textAlign: "center" }}>Quarter</th>
                <th style={{ ...sepTh, textAlign: "left", padding: "5px 14px" }}>Report Date</th>
                <th style={thStyle}>Beat/Miss</th>
                <th style={thStyle}>YoY</th>
                <th style={sepTh}>QoQ</th>
                <th style={thStyle}>Beat/Miss</th>
                <th style={thStyle}>YoY</th>
                <th style={sepTh}>QoQ</th>
                <th style={thStyle}>Beat/Miss</th>
                <th style={thStyle}>YoY</th>
                <th style={thStyle}>QoQ</th>
              </tr>
            </thead>
            <tbody>
              {displayed.map((row, i) => (
                ...
                >
                  ...
                  <td style={{ padding: "8px 14px", fontFamily: "JetBrains Mono, monospace", fontSize: 11, color: TEXT2, whiteSpace: "nowrap", borderRight: "1px solid rgba(15,23,42,0.06)" }}>
                    {row.reportDate}
                  </td>
                  <td style={{ padding: "8px 10px", textAlign: "center", fontFamily: "JetBrains Mono, monospace" }}>{fmtBM(row.revBeatMiss)}</td>
                  <td style={{ padding: "8px 10px", textAlign: "center", fontFamily: "JetBrains Mono, monospace" }}>{fmtGR(row.revYoy)}</td>
                  <td style={{ padding: "8px 10px", textAlign: "center", fontFamily: "JetBrains Mono, monospace", borderRight: "1px solid rgba(15,23,42,0.06)" }}>{fmtGR(row.revQoq)}</td>
                  <td style={{ padding: "8px 10px", textAlign: "center", fontFamily: "JetBrains Mono, monospace" }}>{fmtBM(row.ebitdaBeatMiss)}</td>
                  <td style={{ padding: "8px 10px", textAlign: "center", fontFamily: "JetBrains Mono, monospace" }}>{fmtGR(row.ebitdaYoy)}</td>
                  <td style={{ padding: "8px 10px", textAlign: "center", fontFamily: "JetBrains Mono, monospace", borderRight: "1px solid rgba(15,23,42,0.06)" }}>{fmtGR(row.ebitdaQoq)}</td>
                  <td style={{ padding: "8px 10px", textAlign: "center", fontFamily: "JetBrains Mono, monospace" }}>{fmtBM(row.niBeatMiss)}</td>
                  <td style={{ padding: "8px 10px", textAlign: "center", fontFamily: "JetBrains Mono, monospace" }}>{fmtGR(row.niYoy)}</td>
                  <td style={{ padding: "8px 10px", textAlign: "center", fontFamily: "JetBrains Mono, monospace" }}>{fmtGR(row.niQoq)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 14px", borderTop: "1px solid rgba(15,23,42,0.06)" }}>
            <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <span style={{ width: 8, height: 8, borderRadius: 2, background: GREEN, opacity: 0.8 }} />
                <span style={{ fontSize: 10, color: TEXT2 }}>Beat / Positive</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <span style={{ width: 8, height: 8, borderRadius: 2, background: RED, opacity: 0.8 }} />
                <span style={{ fontSize: 10, color: TEXT2 }}>Miss / Negative</span>
              </div>
...
// ── Tab nav ───────────────────────────────────────────────────────────────────
type QuantTab = "momentum" | "kpi" | "earnings" | "resources";
  ...
  return (
    <>
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-6">
        ...
        {activeTab === "earnings" && <EarningsSurprisesTab />}
