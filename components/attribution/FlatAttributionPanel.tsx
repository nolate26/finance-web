  ...
  const Th = ({ col, label, right }: { col: SortKey; label: string; right?: boolean }) => (
    <th
      ...
      style={{
        ...
        textTransform: "uppercase", fontFamily: FONT_SECONDARY,
        color: sortKey === col ? PATRIA.darkBlue : PATRIA.kingBlue,
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
        <thead>
          <tr>
            ...
            <th style={{ padding: "7px 10px", fontSize: 9, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: PATRIA.kingBlue, fontFamily: FONT_SECONDARY, background: "#F5F7FD", whiteSpace: "nowrap" }}>Company</th>
        ...
        <tbody>
          {sorted.map((row, i) => (
            ...
            >
              {/* Ticker */}
              <td style={{ padding: "8px 10px", fontFamily: FONT_SECONDARY, fontVariantNumeric: "tabular-nums", fontWeight: 700, color: BLUE, fontSize: 11, whiteSpace: "nowrap" }}>
                {row.ticker}
              </td>
              ...
              {/* Port. Weight */}
              <td style={{ padding: "8px 10px", textAlign: "right", fontFamily: FONT_SECONDARY, fontVariantNumeric: "tabular-nums", fontSize: 11, color: TEXT1 }}>
                {fmtW(row.wp)}
              </td>

              {/* Bench. Weight */}
              <td style={{ padding: "8px 10px", textAlign: "right", fontFamily: FONT_SECONDARY, fontVariantNumeric: "tabular-nums", fontSize: 11, color: TEXT2 }}>
                {fmtW(row.wb)}
              </td>
              ...
              <td style={{ padding: "8px 10px", textAlign: "right", fontFamily: FONT_SECONDARY, fontVariantNumeric: "tabular-nums", fontSize: 11, fontWeight: 700 }}>
                <span style={{
                  ...
                  background: row.activeWeight > 0 ? "rgba(0,30,175,0.09)" : row.activeWeight < 0 ? "rgba(248,72,94,0.09)" : "transparent",
                  border: `1px solid ${row.activeWeight > 0 ? "rgba(0,30,175,0.25)" : row.activeWeight < 0 ? "rgba(248,72,94,0.25)" : "transparent"}`,
              ...
              {/* Return */}
              <td style={{ padding: "8px 10px", textAlign: "right", fontFamily: FONT_SECONDARY, fontVariantNumeric: "tabular-nums", fontSize: 11, fontWeight: 600, color: pctColor(row.ret) }}>
                {fmtPct(row.ret)}
              </td>

              {/* Allocation Effect */}
              <td style={{ padding: "8px 10px", textAlign: "right", fontFamily: FONT_SECONDARY, fontVariantNumeric: "tabular-nums", fontSize: 11, fontWeight: 800 }}>
                <span style={{
                  color: pctColor(row.allocationEffect),
                  background: row.allocationEffect > 0 ? "rgba(0,30,175,0.09)" : row.allocationEffect < 0 ? "rgba(248,72,94,0.09)" : "transparent",
                  border: `1px solid ${row.allocationEffect > 0 ? "rgba(0,30,175,0.25)" : row.allocationEffect < 0 ? "rgba(248,72,94,0.25)" : "transparent"}`,
        ...
        {/* Totals row */}
        <tfoot>
          <tr style={{ background: "#F5F7FD", borderTop: "2px solid rgba(13,13,56,0.10)" }}>
            <td colSpan={2} style={{ padding: "8px 10px", fontSize: 10, fontWeight: 700, color: PATRIA.kingBlue, fontFamily: FONT_SECONDARY, letterSpacing: "0.06em", textTransform: "uppercase" }}>
              Total / Weighted Sum
            </td>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        ...
        {/* Benchmark chip */}
        <span style={{
          fontSize: 10, fontWeight: 700, color: "#001EAF",
