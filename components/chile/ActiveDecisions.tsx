...
function Gauge({ low, mid, high, current, signal }: {
  ...
  return (
    <div style={{ position: "relative", height: 22, width: "100%", minWidth: 140 }}>
      <div style={{ position: "absolute", top: 9, left: 0, right: 0, height: 4, borderRadius: 2, background: "rgba(13,13,56,0.06)" }} />
      <div style={{ position: "absolute", top: 8, height: 6, borderRadius: 3, left: `${pct(low)}%`, width: `${pct(high) - pct(low)}%`, background: "rgba(32,68,220,0.16)", border: "1px solid rgba(32,68,220,0.28)" }} />
...
function BandEdit({ band, vals, editable, edited, onChange, onReset }: {
  ...
  if (!editable) {
    return (
      <div style={{ fontFamily: FONT_SECONDARY, fontVariantNumeric: "tabular-nums", fontSize: 11.5 }}>
  ...
  const inp = (f: keyof Band3) => (
    <input type="text" inputMode="decimal" value={vals[f as "lower" | "avg" | "upper"]}
      ...
      style={{ width: 48, padding: "2px 4px", fontSize: 11, fontFamily: FONT_SECONDARY, fontVariantNumeric: "tabular-nums", textAlign: "center",
        border: `1px solid ${edited ? "rgba(32,68,220,0.45)" : BORDER}`, borderRadius: 5,
        background: edited ? "rgba(32,68,220,0.05)" : "#fff", color: TEXT1, outline: "none" }} />
  );
  return (
    <div>
...
export default function ActiveDecisions() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center" style={{ padding: 60 }}>
      <div className="w-8 h-8 rounded-full border-2 animate-spin" style={{ borderColor: "rgba(32,68,220,0.15)", borderTopColor: BLUE }} /></div>;
  }
  if (error || !data) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 12, marginBottom: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          <select value={gicsFilter} onChange={(e) => setGicsFilter(e.target.value)}
            style={{ padding: "6px 10px", borderRadius: 8, border: `1px solid ${BORDER}`, background: "#F5F7FD", fontSize: 12, color: gicsFilter === "all" ? TEXT2 : TEXT1, cursor: "pointer", outline: "none", maxWidth: 200 }}>
          ...
          <button onClick={() => load(true)} disabled={pricesLoading}
            style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: pricesLoading ? "default" : "pointer", color: "#fff", background: BLUE, border: "none", opacity: pricesLoading ? 0.7 : 1, boxShadow: "0 1px 3px rgba(32,68,220,0.30)" }}>
      ...
      {/* ── Table ─────────────────────────────────────────────────────────── */}
      <div style={{ overflowX: "auto", border: `1px solid ${BORDER}`, borderRadius: 12, boxShadow: "0 1px 4px rgba(13,13,56,0.06)" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, minWidth: 1120 }}>
          ...
          <tbody>
            {rows.map((r, idx) => {
              ...
              return (
                <tr key={r.ticker} style={{ background: idx % 2 === 0 ? "#fff" : "rgba(13,13,56,0.022)" }}>
                  {/* Company */}
                  <td style={td}>
                    <div style={{ fontWeight: 600, color: TEXT1 }}>{r.name}</div>
                    <div style={{ fontSize: 10, color: TEXT3, fontFamily: FONT_SECONDARY, fontVariantNumeric: "tabular-nums" }}>{r.ticker.replace(/ EQUITY$/i, "")}</div>
                  </td>
                  {/* GICS */}
                  <td style={{ ...td, fontSize: 11, color: TEXT2, maxWidth: 150 }}>{r.gics ?? "—"}</td>
                  {/* Current */}
                  <td style={{ ...td, textAlign: "right", fontFamily: FONT_SECONDARY, fontVariantNumeric: "tabular-nums", fontWeight: 600, color: r.currentPrice != null ? TEXT1 : TEXT3 }}>{fmtCLP(r.currentPrice)}</td>
                  {/* Where are we */}
                  ...
                  {metric === "pe" ? (
                    <td style={{ ...td, textAlign: "right", fontFamily: FONT_SECONDARY, fontVariantNumeric: "tabular-nums", color: r.netIncome != null ? TEXT1 : TEXT3 }}>
                      {fmtMM(r.netIncome)}
                      {r.niCurrency && <span style={ccyBadge(r.niCurrency)}>{r.niCurrency}</span>}
                    </td>
                  ) : (
                    <>
                      <td style={{ ...td, textAlign: "right", fontFamily: FONT_SECONDARY, fontVariantNumeric: "tabular-nums", color: r.ebitda != null ? TEXT1 : TEXT3 }}>
                        {fmtMM(r.ebitda)}
                        {r.niCurrency && <span style={ccyBadge(r.niCurrency)}>{r.niCurrency}</span>}
                      </td>
                      <td style={{ ...td, textAlign: "right", fontFamily: FONT_SECONDARY, fontVariantNumeric: "tabular-nums", color: r.netDebt != null ? (r.netDebt < 0 ? GREEN : TEXT1) : TEXT3 }}>
                        {fmtBn(r.netDebt)}
                  ...
                  <td style={{ ...td, textAlign: "right", fontFamily: FONT_SECONDARY, fontVariantNumeric: "tabular-nums" }}>
                  ...
                  {/* Implied */}
                  <td style={{ ...td, textAlign: "center", fontFamily: FONT_SECONDARY, fontVariantNumeric: "tabular-nums" }}>
                    {r.implied ? (
      ...
      <div style={{ marginTop: 10, fontSize: 11, color: TEXT2, lineHeight: 1.6 }}>
        ...
        <div><strong>Monedas:</strong> los múltiplos no tienen moneda (son ratios). El NET_INCOME / EBITDA del consenso vienen en la moneda de reporte de la empresa; las que reportan en <span style={{ color: "#FF6B06", fontWeight: 700 }}>USD</span> se detectan y convierten a CLP con USDCLP (badge <span style={{ color: "#FF6B06", fontWeight: 700 }}>USD</span>). El precio está en CLP.</div>
...
const retryBtn: React.CSSProperties = { marginTop: 10, padding: "6px 16px", borderRadius: 6, background: "rgba(32,68,220,0.08)", border: "1px solid rgba(32,68,220,0.20)", color: BLUE, cursor: "pointer", fontSize: 13 };
const ccyBadge = (ccy: "CLP" | "USD"): React.CSSProperties => ({ marginLeft: 4, fontSize: 9, fontWeight: 700, color: ccy === "USD" ? "#FF6B06" : TEXT3, background: ccy === "USD" ? "rgba(255,107,6,0.12)" : "rgba(13,13,56,0.10)", borderRadius: 4, padding: "1px 4px" });
