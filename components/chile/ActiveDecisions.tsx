...
// ni_yr1e (ss_universe) viene en CLP mil-millones → a CLP absoluto.
const NI_SCALE = 1e9;

type Metric  = "pe" | "ev";
type SortKey = "name" | "gics" | "mult" | "ni" | "shares" | "implied" | "current" | "position";
type Band3   = { lower: number; avg: number; upper: number };

// ── Format helpers ──────────────────────────────────────────────────────────────
const fmtMult = (v?: number | null) => (v == null || !isFinite(v) ? "—" : v.toFixed(1) + "x");
const fmtCLP  = (v?: number | null) =>
  v == null || !isFinite(v) ? "—" : v.toLocaleString("es-CL", { maximumFractionDigits: v >= 100 ? 0 : 1 });
...
interface Row extends ADCompany {
  activeBand: ADBand | null;          // banda vigente (editada o calculada)
  edited:     boolean;
  eps:        number | null;          // utilidad por acción (CLP) = ni_yr1e×1e9 / acciones
...
function Gauge({ low, mid, high, current, signal }: {
  ...
  return (
    <div style={{ position: "relative", height: 22, width: "100%", minWidth: 150 }}>
...
// ── Editable multiple band (low / avg / high) ───────────────────────────────────
function BandEdit({ band, editable, onChange, onReset, edited }: {
  band: ADBand; editable: boolean; onChange: (f: keyof Band3, v: string) => void; onReset: () => void; edited: boolean;
}) {
  const inp = (f: keyof Band3) => (
    <input type="number" step={0.5} value={Number(band[f].toFixed(2))}
      onChange={(e) => onChange(f, e.target.value)}
      style={{ width: 46, padding: "2px 4px", fontSize: 11, fontFamily: "JetBrains Mono, monospace", textAlign: "center",
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 10, color: TEXT3, marginTop: 2 }}>n={band.n} · ×/avg/×{edited ? " · editado" : ""}</div>
    </div>
  );
}
...
export default function ActiveDecisions() {
  ...
  // GICS options (entre las que tienen banda del múltiplo activo).
  ...
  // Build computed + filtered + sorted rows.
  const rows = useMemo<Row[]>(() => {
    ...
    for (const co of data?.companies ?? []) {
      ...
      const activeBand: ADBand = ov ? { ...baseBand, ...ov } : baseBand;
      ...
      // Precio implícito solo para P/E (EV/EBITDA: stub). Utilidad CLP / acciones.
      if (metric === "pe" && co.niYr1e != null && co.shares) {
        eps = (co.niYr1e * NI_SCALE) / co.shares;
        const low = activeBand.lower * eps, mid = activeBand.avg * eps, high = activeBand.upper * eps;
        // low > 0 evita bandas sin sentido cuando σ > avg (multiplos muy volátiles).
        if (activeBand.lower > 0 && high > low && eps > 0) {
          implied = { low, mid, high };
          if (co.currentPrice != null) {
            signal = co.currentPrice < low ? "below" : co.currentPrice > high ? "above" : "in";
            position = (co.currentPrice - low) / (high - low);
          }
        }
      }
      list.push({ ...co, activeBand, edited: !!ov, eps, implied, signal, position });
    }
    ...
    const val = (r: Row): number | string | null => {
      switch (sortKey) {
        ...
        case "mult":     return r.activeBand?.avg ?? null;
        case "ni":       return r.niYr1e;
    ...
    return list;
  }, [data, metric, gicsFilter, overrides, sortKey, sortDir]);

  const setBandField = (co: ADCompany, field: keyof Band3, valueStr: string) => {
    const base = metric === "pe" ? co.pe : co.evEbitda;
    if (!base) return;
    const v = parseFloat(valueStr);
    if (isNaN(v)) return;
    const key = `${metric}|${co.ticker}`;
    setOverrides((prev) => {
      const cur = prev[key] ?? { lower: base.lower, avg: base.avg, upper: base.upper };
      return { ...prev, [key]: { ...cur, [field]: v } };
    });
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 12, marginBottom: 12, flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: TEXT2, margin: "3px 0 0 13px" }}>
            Precio actual vs. rango implícito por múltiplo histórico (±1σ, 10A)
            {data.asOf ? ` · valuación al ${data.asOf}` : ""}{data.ssAsOf ? ` · fundamentales ss_universe al ${data.ssAsOf}` : ""}
          </p>
        </div>
      ...
      {metric === "ev" && (
        <div style={banner("#B45309")}>EV/EBITDA: por ahora solo se muestra la banda del múltiplo. El precio implícito (EV − deuda neta) / acciones queda pendiente.</div>
      )}
      {metric === "pe" && !priced && (
        <div style={{ fontSize: 11.5, color: TEXT2, marginBottom: 10 }}>
          Banda del múltiplo y utilidad cargadas. Apreta <strong>Fetch current prices</strong> para traer precio y acciones (Yahoo) y ver el precio implícito y dónde estamos.
        </div>
      )}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, minWidth: 1080 }}>
          <thead>
            <tr style={{ background: "rgba(248,250,255,0.95)" }}>
              ...
              <Th label="Utilidad (CLP bn)" k="ni" {...{ sortKey, sortDir, sortBy }} align="right" />
          ...
          <tbody>
            {rows.map((r, idx) => (
              <tr key={r.ticker} style={{ background: idx % 2 === 0 ? "#fff" : "rgba(248,250,255,0.5)" }}>
                ...
                <td style={td}>
                  <BandEdit band={r.activeBand!} editable={metric === "pe"} edited={r.edited}
                    onChange={(f, v) => setBandField(r, f, v)} onReset={() => resetBand(r)} />
                </td>
                <td style={{ ...td, textAlign: "right", fontFamily: "JetBrains Mono, monospace", color: r.niYr1e != null ? TEXT1 : TEXT3 }}>{fmtBn(r.niYr1e)}</td>
                <td style={{ ...td, textAlign: "right", fontFamily: "JetBrains Mono, monospace" }}>
                  ...
                  {r.eps != null && <div style={{ fontSize: 10, color: TEXT3 }}>EPS {fmtCLP(r.eps)}</div>}
                </td>
                <td style={{ ...td, textAlign: "center", fontFamily: "JetBrains Mono, monospace" }}>
                  ...
                  ) : <span style={{ color: TEXT3 }}>{metric === "ev" ? "Próximamente" : r.eps != null ? "σ alta · editá low" : priced ? "—" : "Fetch prices →"}</span>}
                </td>
                <td style={{ ...td, textAlign: "right", fontFamily: "JetBrains Mono, monospace", color: r.currentPrice != null ? TEXT1 : TEXT3 }}>{fmtCLP(r.currentPrice)}</td>
                <td style={{ ...td, minWidth: 200 }}>
                  ...
                  ) : <span style={{ color: TEXT3, fontSize: 11 }}>{metric === "ev" ? "—" : r.eps != null ? "σ alta · editá low" : priced ? "—" : "Fetch prices →"}</span>}
                </td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr><td colSpan={8} style={{ textAlign: "center", padding: 32, color: TEXT3, fontSize: 13 }}>
      ...
      <div style={{ marginTop: 10, fontSize: 11, color: TEXT2, lineHeight: 1.6 }}>
        <div><strong>{rows.length} empresas.</strong> <strong>avg</strong> = promedio histórico del múltiplo forward (10A); la <strong>banda</strong> es avg ±1σ (±1 desviación estándar); <strong>n</strong> = nº de observaciones mensuales usadas.</div>
        <div><strong>Precio implícito</strong> = múltiplo × utilidad por acción, donde utilidad por acción (EPS) = <code>ni_yr1e</code> (ss_universe) / acciones. Podés <strong>editar</strong> los múltiplos (low/avg/high) si no estás de acuerdo y el implícito se recalcula.</div>
        <div><strong>Monedas:</strong> los múltiplos no tienen moneda (son ratios). La utilidad (<code>ni_yr1e</code>) y el precio están ambos en <strong>CLP</strong>, así que no hay conversión de monedas.</div>
...
const banner = (c: string): React.CSSProperties => ({ fontSize: 11.5, color: c, background: `${c}12`, border: `1px solid ${c}33`, borderRadius: 8, padding: "7px 12px", marginBottom: 10 });
